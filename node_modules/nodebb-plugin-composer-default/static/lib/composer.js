...
define('composer', [
	...
	async function createNewComposer(post_uuid) {
		...
		//groupOptions = await groups.getUserGroupMembership('groups:visible:createtime', [postData.uid]);
		//console.log("in composer.js..");
		...
		$('.selecting-group').on('click', function () {
	...
	async function post(post_uuid) {
		...
		var selectedGroup = postContainer.find('#groups').val();
